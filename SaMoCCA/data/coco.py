...
class DatasetCOCO(Dataset):
    def __init__(self, datapath, fold, transform, split, shot, use_original_imgsize, use_saliency=True, img_size=384):
        ...
        self.sal_path = os.path.join(self.base_path, 'saliency')
    ...
    def __getitem__(self, idx):
        ...
        support_saliencies = torch.stack([self.read_saliency(name, support_imgs.size()[-2:]) for name in support_names])
        ...
        batch = {'query_img': query_img,
                 ...
                 'support_saliencies': support_saliencies,
                 'support_names': support_names,
                 'class_id': torch.tensor(class_sample)}

        return batch

    def read_saliency(self, name, size):
        r""" Return dense class-agnostic saliency in [0, 1], resized to `size`. """
        sal_file = os.path.join(self.sal_path, os.path.splitext(name)[0] + '.png')
        if not self.use_saliency or not os.path.exists(sal_file):
            return torch.zeros(size)
        sal = torch.tensor(np.array(Image.open(sal_file).convert('L'))).float() / 255.0
        sal = F.interpolate(sal.unsqueeze(0).unsqueeze(0), size=tuple(size), mode='bilinear',
