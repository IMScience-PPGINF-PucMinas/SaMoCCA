r""" SaMoCCA: Saliency-guided Model with Cross and Channel Attention for Few-Shot Segmentation """
...
from model.base.transformer import MultiHeadedAttention, PositionalEncoding, SqueezeAndExcitation


class Ensemble(nn.Module):
    r""" Fuse the binary support mask with the dense class-agnostic saliency map.

    Implements M^s = Conv_{1x1}(norm(S^s) + M^s), where norm() rescales the
    saliency to [0, 1] (per-sample min-max) and the 1x1 conv mixes the two
    single-channel signals into a refined, continuous support mask.
    """
    def __init__(self):
    ...
    def forward(self, mask, saliency):
        r""" mask, saliency: [B, H, W] -> fused mask: [B, H, W] """
        m = mask.unsqueeze(1).float()
        s = self._norm01(saliency.unsqueeze(1).float())
        fused = self.conv(s + m)
        return fused.squeeze(1)


class SaMoCCA(nn.Module):

    def __init__(self, backbone, pretrained_path, use_original_imgsize):
        super(SaMoCCA, self).__init__()
        ...
        self.model = SaMoCCA_model(in_channels=self.feat_channels, stack_ids=self.stack_ids)

        self.cross_entropy_loss = nn.CrossEntropyLoss()

    def forward(self, query_img, support_img, support_mask, support_saliency):
        with torch.no_grad():
            query_feats = self.extract_feats(query_img)
            support_feats = self.extract_feats(support_img)

        logit_mask = self.model(query_feats, support_feats, support_mask.clone(), support_saliency.clone())
    ...
    def predict_mask_nshot(self, batch, nshot):
        ...
        support_saliencies = batch['support_saliencies']

        if nshot == 1:
            logit_mask = self(query_img, support_imgs[:, 0], support_masks[:, 0], support_saliencies[:, 0])
        else:
            with torch.no_grad():
                query_feats = self.extract_feats(query_img)
                n_support_feats = []
            ...
            logit_mask = self.model(query_feats, n_support_feats, support_masks.clone(),
                                    support_saliencies.clone(), nshot)
...
class SaMoCCA_model(nn.Module):
    def __init__(self, in_channels, stack_ids):
        super(SaMoCCA_model, self).__init__()

        self.stack_ids = stack_ids

        # Ensemble: fuse support mask with class-agnostic saliency
        self.ensemble = Ensemble()

        # Multi-layer cross-attention blocks (1/8, 1/16, 1/32) + positional encoding
        ...
        # Squeeze-and-excitation block on the coarse merge branch (channel recalibration)
        self.se = SqueezeAndExcitation(outch3, ratio=16)

        # conv blocks (Conv block x3 per scale)
    ...
    def forward(self, query_feats, support_feats, support_mask, support_saliency, nshot=1):
        # Ensemble: fuse the support mask with saliency (saliency only used for support)
        if nshot == 1:
            support_mask = self.ensemble(support_mask, support_saliency)
        else:
            bsz_n, shot_n = support_mask.size(0), support_mask.size(1)
            fused = self.ensemble(support_mask.flatten(0, 1), support_saliency.flatten(0, 1))
            support_mask = fused.view(bsz_n, shot_n, *fused.size()[1:])
        ...
        for idx, query_feat in enumerate(query_feats):
            ...
            # multi-layer cross-attention forward (scaled dot-product, mask-guided)
        ...
        coarse_masks1 = self.conv1(coarse_masks1)  # 1/32
        coarse_masks2 = self.conv2(coarse_masks2)  # 1/16
        coarse_masks3 = self.conv3(coarse_masks3)  # 1/8

        # coarse merge 1/32 -> 1/16, then channel recalibration (SE residual)
        ...
        # mixer blocks forward (bilinear upsampling 1/4 -> 1/2 -> original)
        out = self.mixer1(mix)
